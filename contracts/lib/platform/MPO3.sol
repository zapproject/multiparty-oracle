...
// @title - A MultiPartyOracle contract that implements the client3 callback
...
contract MPO3 is OnChainProvider, Client3{
    ...
    event Result3(uint256 id, string response1, string response2, string response3);
    ...
    function callback(uint256 queryId, string response, string response2, string response3) external {
        ...
        if(stor.getQueryStatus(stor.getClientQueryId(queryId)) == 1){
            ...
            if(stor.getTally(stor.getClientQueryId(queryId), response) >= stor.getThreshold()) {
                ...
                emit Result3(stor.getClientQueryId(queryId), response, response2,response3);
                dispatch.respond3(stor.getClientQueryId(queryId), response, response2,response3);
            }
        }
            ...
            if(stor.getTally(stor.getClientQueryId(), response) >= stor.getThreshold()) {
                ...
                emit Result3(stor.getClientQueryId(), response, response2, response3);
                dispatch.respond3(stor.getClientQueryId(), response, response2, response3);
            }
