...
// @title - A MultiPartyOracle contract that implements the client2 callback
...
contract MPO2 is OnChainProvider, Client2{
    ...
    function callback(uint256 queryId, string response, string response2) external {
        ...
        if(stor.getQueryStatus(stor.getClientQueryId(queryId)) == 1){
            ...
            if(stor.getTally(stor.getClientQueryId(queryId), response) >= stor.getThreshold()) {
                ...
                emit Result2(stor.getClientQueryId(queryId), response, response2);
                dispatch.respond2(stor.getClientQueryId(queryId), response, response2);
            }
        }
            ...
            if(stor.getTally(stor.getClientQueryId(), response) >= stor.getThreshold()) {
                ...
                emit Result2(stor.getClientQueryId(), response, response2);
                dispatch.respond2(stor.getClientQueryId(), response, response2);
            }
